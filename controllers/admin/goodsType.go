...
func (con GoodsTypeController) DoAdd(c *gin.Context) {
	...
	if title == "" {
		con.Error(c, "商品类型的标题不能为空", "/admin/goodsType/add")
		return
	}
	...
	if err != nil {
		con.Error(c, "添加商品类型失败，请重试！", "/admin/goodsType/add")
	} else {
		con.Success(c, "添加商品类型成功", "/admin/goodsType")
	}
...
func (con GoodsTypeController) DoEdit(c *gin.Context) {
	id, err1 := models.Int(c.PostForm("id")) //获得的是提交表单的参数
	title := strings.Trim(c.PostForm("title"), " ")
	description := strings.Trim(c.PostForm("description"), " ")
...
func (con GoodsTypeController) Delete(c *gin.Context) {
	...
	if err != nil {
		con.Error(c, "删除数据错误", "/admin/goodsType")
	} else {
		goodsType := models.GoodsType{Id: id}
